...
class push(Unit):
    ...
    will output `key=censored`. The application of `refinery.rex` turns the (duplicated) data into
    just the value, which is then stored in the variable `v`. Next, application of `refinery.repl`
    replaces this value with the hard-coded string `censored`.
    """
    def __init__(self, data: Param[buf,
        Arg(help='The data to be pushed, by default a copy of the input.')] = B''
    ):

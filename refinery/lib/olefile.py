"""
Read-only parser for Microsoft OLE2 Compound Binary Files (CFB).
"""
...
class DirectoryEntry:
    ...
    def __init__(
        ...
        try:
            self.name = name_bytes.decode('utf-16-le')
        except UnicodeDecodeError:
            self.name = name_bytes.decode('utf-16-le', errors='replace')
...
def _read_chain(
    ...
    for _ in range(nb_sectors):
        if sect >= len(fat) or sect > MAXREGSECT:
            break
        if sect in visited:
            break
        visited.add(sect)
...
class OleFile:
    """
    Read-only parser for OLE2 Compound Binary Files.
    """

    def __init__(self, data: bytes | bytearray | memoryview | MemoryFile):
        if isinstance(data, MemoryFile):
            ...
            raise TypeError(F'Expected bytes, bytearray, memoryview, or MemoryFile, got {type(data).__name__}')
    ...
    def _load_directory(self):
        ...
        for sid in range(max_entries):
            ...
            entry = DirectoryEntry(
                sid, chunk, self._sector_size, self._mini_stream_cutoff)
            entries[sid] = entry
    ...
    def _open_stream(self, entry: DirectoryEntry) -> MemoryFile:
        if entry.is_minifat:
    ...
    def _find(self, filename: str | list[str]) -> DirectoryEntry | None:
    ...
    def openstream(self, filename: str | list[str]) -> MemoryFile:
    ...
    def exists(self, filename: str | list[str]) -> bool:
        return self._find(filename) is not None

    def get_type(self, filename: str | list[str]) -> int:
        entry = self._find(filename)
        if entry is None:
            return STGTY.EMPTY
    ...
    def getclsid(self, filename: str | list[str]) -> str:
        entry = self._find(filename)
        if entry is None:
            raise OleFileError(F'Entry not found: {filename!r}')
        return entry.clsid_str

    def getmtime(self, filename: str | list[str]) -> datetime.datetime | None:
        entry = self._find(filename)
        if entry is None:
            return None
        return filetime_to_datetime(entry.modify_time)

    def getctime(self, filename: str | list[str]) -> datetime.datetime | None:
        entry = self._find(filename)
        if entry is None:
            return None

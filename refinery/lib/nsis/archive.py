...
class Op(enum.IntEnum):
    ...
    DetailPrint        = enum.auto()    # noqa; DetailPrint
    ...
    ExecShell          = enum.auto()    # noqa; ExecShell
    Exec               = enum.auto()    # noqa; Exec, ExecWait
...
#   GetFontName        = enum.auto()    # noqa; Park : 2.46.3
    ...
    DeleteReg          = enum.auto()    # noqa; DeleteRegValue, DeleteRegKey
    WriteReg           = enum.auto()    # noqa; WriteRegStr, WriteRegExpandStr, WriteRegBin, WriteRegDWORD
    ReadReg            = enum.auto()    # noqa; ReadRegStr, ReadRegDWORD
    EnumReg            = enum.auto()    # noqa; EnumRegKey, EnumRegValue
...
class NSSectionFlags(enum.IntFlag):
    SELECTED   = 1 << 0  # noqa
    SECGRP     = 1 << 1  # noqa
    SECGRPEND  = 1 << 2  # noqa
    BOLD       = 1 << 3  # noqa
...
class NSHeader(Struct):
    ...
    def __init__(
        ...
        if self.is64bit:
            log('64bit archive detected')
        bho_size = 12 if self.is64bit else 8
        required = 4 + bho_size * 8
        if size < required:
            raise ValueError(
                F'Invalid size {size} was specified for NSIS main header, needs to be at least {required}.')
        self.unknown_value = reader.u32()
        self.block_header_offsets = [
            NSBlockHeaderOffset.Parse(reader.read(bho_size), is64bit=self.is64bit) for _ in range(8)]
        ...
        self.bh_pages = self.block_header_offsets[0]
        self.bh_sections = self.block_header_offsets[1]
        self.bh_ctlcolors = self.block_header_offsets[5]
        self.bh_font = self.block_header_offsets[6]
        self.bh_data = self.block_header_offsets[7]
        ...
        if self.bh_entries.offset > size:
            raise ValueError(
                F'Invalid NSIS header: Size is 0x{size:08X}, '
                F'but entries block header offset is 0x{self.bh_entries.offset:08X}.')
        if self.bh_strings.offset > size:
            raise ValueError(
                F'Invalid NSIS header: Size is 0x{size:08X}, '
                F'but strings block header offset is 0x{self.bh_strings.offset:08X}.')
        if self.bh_langtbl.offset > size:
            raise ValueError(
                F'Invalid NSIS header: Size is 0x{size:08X}, '
                F'but language list header offset is 0x{self.bh_langtbl.offset:08X}.')
        ...
        if string_table_size < 2:
            raise ValueError(
                F'The calculated string table size is {string_table_size}, too small to parse.')
        ...
        self.raw_data = memoryview(reader.getbuffer())
        self.raw_size = size
    ...
    def opcode(self, cmd: NSScriptInstruction | NSScriptExtendedInstruction) -> Op:
        code = cmd.opcode
        if self.type < NSType.Park1:
    ...
    def _parse_sections(self) -> list[NSSection]:
        sections: list[NSSection] = []
        bh = self.bh_sections
        if bh.count == 0 or self.bh_entries.offset <= bh.offset:
            return sections
        section_size = (self.bh_entries.offset - bh.offset) // bh.count
        if section_size < 24:
            return sections
        reader = StructReader(self.raw_data)
        for i in range(bh.count):
            reader.seekset(bh.offset + i * section_size)
            sections.append(NSSection(
                name=reader.u32(),
                install_types=reader.u32(),
                flags=reader.u32(),
                start_cmd_index=reader.u32(),
                num_commands=reader.u32(),
                size_kb=reader.u32(),
            ))
        return sections
    ...
    @property
    def script(self):
        from refinery.lib.nsis.decompiler import NSDecompiler
        return NSDecompiler(self).decompile()
...
class NSArchive(Struct):
    ...
    def __init__(
        ...
        errors = min(
            sum(1 for a, b in zip(self.signature, sig) if a != b) for sig in self.MAGICS)
        if errors > 3:
            raise ValueError(
                F'Out of {0x10} bytes, there were {errors} errors in the signature.')
        ...
        log(F'size of headers: {header_size}')
        log(F'size of archive: {archive_size}')

        if body_size < 0:
            raise ValueError(
                'Header indicates that the archive size is less than the header size.')
        ...
        log('header preview:', lambda: reader.peek(16).hex(' ', 1).upper())
        ...
        if preview_check >= 0:
            ...
            if not preview_check:
                ...
                raise ValueError(
                    F'Found header length at unexpected offset {preview_check}; '
                    F'unknown NSIS format.')
        ...
        log(F'compression: {dbg_method}')
        log(F'archive is solid: {self.solid}')
        ...
        if header_data is None:
            ...
            if header_entry.decompression_error:
                raise NotImplementedError(
                    'This archive seems to use an NSIS-specific deflate algorithm '
        ...
        log(F'read header of length {len(header_data)}')

        self.header_data = header_data
        self.header = NSHeader.Parse(header_data, size=header_size, extended=self.extended, log=log)
        self.reader = reader
    ...
    def script(self):
        from refinery.lib.nsis.decompiler import NSDecompiler
        return NSDecompiler(self.header, self).decompile()
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        raise LookupError(
                            F'No data for item {item!s} could not be found.')
    ...
    class SolidReader(Iterable[Entry]):
        ...
        def __next__(self):
            ...
            if len(data) != read:
                raise EOFError(
                    'Unexpected end of stream while decompressing archive entries.')
    ...
    class PartsReader(SolidReader):
        def __init__(
            self,
            src: BinaryIO,
            decompressor: type[BinaryIO] | None,
            prefix_length: int,
        ):
    ...
    def _decompress_items(
        self, reader: StructReader[bytearray],
    ) -> Iterator[NSArchive.Entry]:
        def NSISLZMAFile(d: StructReader[bytearray]):
            if use_filter := self.lzma_options.filter_flag:
                use_filter = d.u8()
            if use_filter > 1:
                raise ValueError(
                    F'LZMA/BCJ chunk with invalid filter indicator byte 0x{use_filter:X}')
            if not use_filter:
                _filter = None
                _format = None

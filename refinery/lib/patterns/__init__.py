...
_suffix_combined_url = R'([/?#](?:[#/=:;$!?&.,\w\+\%\-\*\'~@()](?![a-zA-Z0-9][-a-zA-Z0-9]{0,20}://))*)?'
...
class wallets(_PatternEnum):
    ...
    BTC = pattern("(?:[13][a-km-zA-HJ-NP-Z1-9]{25,34}|bc1[a-z0-9]{25,59})")
    DASH = pattern("X[1-9A-HJ-NP-Za-km-z]{33}")
    DOGE = pattern("D{1}[5-9A-HJ-NP-U]{1}[1-9A-HJ-NP-Za-km-z]{32}")
    DOT = pattern("1[0-9a-zA-Z]{47}")
    ETH = pattern("0x[0-9a-fA-F]{40}")
    IOTA = pattern("iota[a-z0-9]{10,}")
    LSK = pattern("[0-9]{19}L")
    LTC = pattern("(?:[LM3][a-km-zA-HJ-NP-Z1-9]{26,33}|ltc1[a-z0-9]{25,87})")
    NEO = pattern("N[1-9A-HJ-NP-Za-km-z]{33}")
    ONE = pattern("(?:bnb|one)1[a-z0-9]{38}")
    ONT = pattern("A[1-9A-HJ-NP-Za-km-z]{33}")
    RONIN = pattern("ronin:[0-9a-fA-F]{40}")
    RVN = pattern("R[1-9A-HJ-NP-Za-km-z]{33}")
    SOLANA = pattern("[1-9A-HJ-NP-Za-km-z]{43,44}")
    TERRA = pattern("terra1[a-z0-9]{38}")
    TON = pattern("[UE][Qf][0-9a-zA-Z_-]{46}")
    TRON = pattern("T[A-Za-z1-9]{33}")
    WIF = pattern("5[HJK][1-9A-HJ-NP-Za-km-z]{49}")
    XEM = pattern("N[A-Z2-7]{39}|N[A-Z2-7]{4,7}(?:-[A-Z2-7]{4,7}){4,6}")

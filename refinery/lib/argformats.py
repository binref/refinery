...
class DelayedArgument(LazyEvaluation):
    ...
    @handler.register('rand', final=True)
    def rand(
        self,
        type: str,
        seed: str | None = None,
        ...
        The final handler
            ...
            rand[seed=0,skip=1]:type

        expects `type` to be one of the following names of common random number genertors:

        - `ansi`: `S * 0x041C64E6D + 0x003039`, yield `S >> 16`
        - `msvc`: `S * 0x0000343FD + 0x269EC3`, yield `S >> 16`
        - `libc`: `S * 0x041C64E6D + 0x003039`, yield `S`
        - `msvb`: `S * 0x043FD43FD + 0xC39EC3`, yield `S`
        - `java`: `S * 0x5DEECE66D + 0x00000B`, yield `S`

        The optional parameters `seed` and `skip` are Python expressions. It generates the infinite
        integer sequence maintaining an internal state `S` that would be generated by one of these
        random number generators. The internal state is initialized to `seed` and is updated `skip`
        times before output is produced.
        """
        try:
            precision, rand, feed = {
                'ansi': (32, 'S * 0x041C64E6D + 0x003039', 'S >> 16'),
                'msvc': (32, 'S * 0x0000343FD + 0x269EC3', 'S >> 16'),
                'libc': (32, 'S * 0x041C64E6D + 0x003039', 'S'),
                'msvb': (32, 'S * 0x043FD43FD + 0xC39EC3', 'S'),
                'java': (64, 'S * 0x5DEECE66D + 0x00000B', 'S'),
            }[type]
        except KeyError:
            raise ArgumentTypeError(F'The generator type {type} is unknown.')
        _rand = PythonExpression.Lazy(rand)
        _seed = PythonExpression.Lazy(seed or '0')
        _feed = PythonExpression.Lazy(feed)
        _skip = 1 if skip is None else int(skip, 0)
        _mask = (1 << precision) - 1

        def finalize(data: Chunk | None = None):
            meta = {} if data is None else dict(metavars(data))
            S: int = _seed(meta)
            F: int = _feed(meta, S=S)
            K: int = _skip
            while True:
                if not K:
                    yield F

...
class ByteStringWrapper(bytearray, CustomStringRepresentation):
    ...
    @classmethod
    def Wrap(cls, string: Union[str, ByteStr, ByteStringWrapper], codec: Optional[str] = None):
        if isinstance(string, cls):
            return string
        return cls(string, codec=codec)

    def __init__(self, string: Union[str, ByteStr], codec: Optional[str] = None):
...
class LazyMetaOracle(metaclass=_LazyMetaMeta):
    ...
    def __init__(self, chunk: ByteStr, scope: Optional[int] = 1, seed: Optional[Dict[str, List[Tuple[bool, Any]]]] = None):
        self.ghost = False
        self.chunk = chunk
        self.cache = {}
        self.index = None
...
def metavars(chunk: Union[Chunk, ByteStr]) -> LazyMetaOracle:
    """

...
class TestArgumentFormats(TestBase):
    ...
    def test_msvc_01(self):
        self.assertEqual(
            argformats.multibin('itob[1]:take[:16]:rand:msvc'),
            bytes.fromhex('26 27 F6 85 97 15 AD 1D D2 94 DD C4 76 19 39 31')
        )

    def test_msvc_02(self):
        pl = self.load_pipeline('emit rep[32]:H:00 [| put s 0xF23CA2 | xor -B2 rand[s]:msvc ]')
        self.assertEqual(pl(),
    ...
    def test_itob(self):
        data = argformats.DelayedArgument('itob:range:0x1337:0x133A')()
        self.assertEqual(data, bytes.fromhex('371338133913'))

...
def _removing_a_handler_asked_at_the_handler(
    self: faults.Ps1FaultReach, handler: Node, may_raise, soft_step_over_observed=lambda _h: True,
) -> bool:
    """
    ...
    or about what they would fall back to. The injected raiser and soft-step-over predicates the real
    transpose takes are accepted and ignored, so the mutation is noticed as a wrong answer rather than
    a broken call.
    """

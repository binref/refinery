...
class TestSwap(TestUnitBase):
    ...
    def test_swap_variable_01(self):
        pl = L('emit BAZ [') | L('put BAR S:FOO') | L('put FOO S:BAR') | L('swap FOO BAR') | L('pf {FOO}{BAR} ]')
        self.assertEqual(pl(), B'FOOBAR')

    def test_swap_variable_02(self):
        pl = L('emit BAZ [') | L('put BAR FOO') | L('swap BAR FOO') | L('pf {FOO} ]')
        self.assertEqual(pl(), B'FOO')

    def test_swap_with_data(self):
        pl = L('emit BAR [') | L('put BAR FOO') | L('swap BAR') | L('pf {}{BAR}{FOO} ]')

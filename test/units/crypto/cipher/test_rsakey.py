...
class TestRSAKeyParser(TestUnitBase):
    ...
    def test_ms_key_private_blob(self):
        key = self.KEY_MS | self.load('json') | json.loads
        self.assertEqual(key['Exponent'], self.KEY_E)
        self.assertEqual(int(key['Modulus'], 16), self.KEY_N)

    def test_cycle_formats(self):
        key = self.KEY_MS | self.load('pem') | self.load('der') | self.load('blob') | bytes
        self.assertEqual(self.KEY_MS, key)

    def test_text_export(self):
        key = self.KEY_MS | self.load('text') | str
        self.assertIn('0x8DBB38F2D3A9600EFC39DD7198F950D9862FAEE1D0466AE9CB3E7D57CDA9EA1287C65DC77B950B', key)
        self.assertIn('0x1EA229C3EA55C08A5C9FD12A1D6B3DBB565276151EA429B590955818233EA775B76CFAAA0B21B1', key)

    KEY_MS = data = bytes.fromhex(
    ...
    KEY_N = int(
        '8DBB38F2D3A9600EFC39DD7198F950D9862FAEE1D0466AE9CB3E7D57CDA9EA1287C65DC77B950B8CD52CEB6D10142AE7'
        '7E244CD11C87BF325C106FDBEF51DF6B', 16)
    KEY_E = 0x10001
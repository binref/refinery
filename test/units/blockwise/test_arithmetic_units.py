...
class TestArguments(TestUnitBase):
    ...
    def test_binary_instrumented(self):
        add = refinery.add(arg=[b'\x12'])
        sub = refinery.sub(arg=[b'\x12'])
...
class TestArithmeticUnits(TestUnitBase):
    ...
    def test_handler_unpack_01(self):
        add = refinery.add('-B3', 'unpack:#2:H:BEEF')
    ...
    def test_handler_unpack_03(self):
        add = refinery.add('-B3', 'unpack:#2:H:BEEF', little_endian=False)
    ...
    def test_argument_overflow(self):
        for blocksize in (1, 2, 3, 4, 5, 7, 8, 11):
            ...
            for unit in (refinery.add, refinery.sub, refinery.xor):
                op1 = unit(str(key1), blocksize=blocksize)
                op2 = unit(str(key2), blocksize=blocksize)
...
class TestBlockopInput(TestUnitBase):
    ...
    def test_invalid_expression(self):
        self.assertRaises(Exception, refinery.blockop, '"§$%$$$§$§$$$UU')
        self.assertRaises(Exception, refinery.blockop, '(B + 9')

    def test_not_an_expression(self):
        self.assertRaises(Exception, refinery.blockop, 'def foo(x):\n    return 29')

    def test_invalid_symbols(self):
        self.assertRaises(Exception, refinery.blockop, '(B ^ 0xFF - Arg)')
        self.assertRaises(Exception, refinery.blockop, '(B ^ 0xFF - a)')
        self.assertRaises(Exception, refinery.blockop, 'B ^ 0x34 - A + W[2]')

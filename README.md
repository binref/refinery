...
```
Here, both `cut:0:16` and `PBKDF2[32,s4lty]:swordfish` are multibin arguments that use a special handler. In this case, `cut:0:16` extracts the slice `0:16` (i.e. the first 16 bytes) from the input data - after application of this multibin handler, the input data has the first 16 bytes removed and the argument `iv` is set to these exact 16 bytes. The handler `PBKDF2[32,s4lty]` on the other hand instructs refinery to create an instance of the PBKDF2 unit as if it were given the command line parameters `32` and `s4lty` in this order and process the byte string `swordfish` with this unit. To intentionally reduce the possible complexity, there is no way to escape the comma character here which serves as the parameter separator. This unit is then used to process the remaining. As a simple test, the following pipeline will encrypt and decrypt a sample piece of text:
```
